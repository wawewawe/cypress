...
export const calculateTestStatus = function (strategy?: 'detect-flake-and-pass-on-threshold' | 'detect-flake-but-always-fail', options?: any) {
  ...
  // Additionally, if the current test passed/failed, add it to the attempt list
  ...
  if (failedTests.length > 0) {
    ...
    if (strategy !== 'detect-flake-but-always-fail' && passingAttempts >= passesRequired) {
      ...
      this.final = true
      // If an outerStatus is 'failed', but the last test was 'passed', we need to force the status so mocha doesn't flag the test as failed.
