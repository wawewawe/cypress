  ...
  _runner.hook = $utils.monkeypatchBefore(_runner.hook, function (name, fn) {
    ...
    const newArgs = [name, $utils.monkeypatchBeforeAsync(fn,
      async function () {
        ...
        if (test.final !== false) {
          test.final = true
          // If the last test passed, but the outerStatus isn't marked as failed, then we want to emit the mocha 'pass' event.
...
const setHookFailureProps = (test, hook, err) => {
  ...
  // Therefore, if the last test fails on a hook, the outerStatus should also indicate a failure.
  ...
  create: (specWindow, mocha, Cypress, cy, state) => {
    ...
    const maybeHandleRetryOnFailure = (runnable, err) => {
      ...
      if (isBeforeEachHook || isAfterEachHook) {
        if (err) {
          // If the beforeEach/afterEach hook failed, mark the test as failed
          test.state = 'failed'
        }

        // Then calculate the test status, accounting for the updated state if the hook errored
        // to see if we should continue running the tests.
        ...
        if (isAfterEachHook) {
          ...
          if (status.shouldAttemptsContinue) {
            ...
            /**
             * OR in the event the test 'passed' and hit the exit condition,
             * BUT the afterEach hook errored which MIGHT change the test exit condition (as the test is now 'failed')
             *
      ...
      if (isTest) {
        // If there is no error on the test, then the test passed!
      ...
      if (err) {
        ...
        if (willRetry && isAfterEachHook) {
          ...
          // Check to see if the test maybe passed, but hasn't satisfied its retry config yet.
          // In this case, we DON't need to add the new test as it is already queued to rerun.
